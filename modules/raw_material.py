...
class RawMaterials(InventoryItem):
    def __init__ (
        self , 
        name , 
        code , 
    ...
    def read_all(self):
        try:
            data = self.loadData()
            dataFiltered = list(filter(lambda x: x['type'] == 'raw', data))
            return dataFiltered
        except json.JSONDecodeError:
                return 0
    ...
    def delete(self , pick ):
        ...
        dataPicked = data[pick - 1 ]['id']
        prevData = self.loadData()
        dataFiltered = list(filter(lambda x: x['type'] == 'raw',prevData))
        dataFilteredPrev = list(filter(lambda x: x['type'] == 'finished',prevData))
